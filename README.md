...
1) `composer require arrilot/bitrix-migrations`, файл migrator копируется в папку vendor/bin. 

2) В файл composer.json в параметр extra добавляем настройки для файла migrator. Например:
...
`public-dir` - это корневая директория сайта.

Блок `arrilot-bitrix-migrations` содержит настройки для массива $config в файле migrator.

3) `php vendor/bin/migrator install`
...
2) Файлы миграций будут создаваться в директории `./migrations` относительно файла migrator, если не указан параметр `extra.arrilot-bitrix-migrations.dir`.
...
1) Создаем файл (или файлы) миграции при помощи `php vendor/bin/migrator make название_миграции`
...
3) Применяем имеющиеся миграции - `php vendor/bin/migrator migrate`
...
Список доступных команд можно получить в консоли - `php vendor/bin/migrator list`

<table>
<tr><th>Название</th><th>Описание</th></tr>
<tr>
    <td>`php vendor/bin/migrator install`</td>
    <td>Создает таблицу для хранения миграций. Запускается один раз.</td>
</tr>
<tr>
    <td>`php vendor/bin/migrator make название_миграции`</td>
    <td>
...
При генерации файла миграции можно указать его шаблон: `php vendor/bin/migrator make название_миграции -t add_iblock` где `add_block` - название шаблона.
...
6) `php vendor/bin/migrator status` - показывает доступные для выполнения миграции, а также последние выполненные.
...
2) Создается файл миграции как при `php vendor/bin/migrator make`
...
и было показано уведомление о том что миграция создана, необходимо вручную откатить такую миграцию при помощи `php vendor/bin/migrator rollback --hard --delete` *
